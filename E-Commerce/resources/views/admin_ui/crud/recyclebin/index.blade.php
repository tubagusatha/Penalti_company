...
<nav class="fixed z-30 w-full bg-white border-b border-gray-200 dark:bg-gray-800 dark:border-gray-700">
                    ...
                    <nav class="flex mb-5" aria-label="Breadcrumb">
                        <ol class="inline-flex items-center space-x-1 text-sm font-medium md:space-x-2">
                          ...
                          <li>
                            <div class="flex items-center">
                              <svg class="w-6 h-6 text-gray-400" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z" clip-rule="evenodd"></path></svg>
                              <span class="ml-1 text-gray-400 md:ml-2 dark:text-gray-500" aria-current="page">Recycle Bin Products</span>
                            </div>
                          </li>
                        </ol>
                    </nav>
                    <h1 class="text-xl font-semibold text-gray-900 sm:text-2xl dark:text-white">Recycle Bin Products</h1>
                </div>
            ...
            <div class="overflow-x-auto">
                <div class="inline-block min-w-full align-middle">
                    <div class="overflow-hidden shadow">
                        <table id="crudTable" class="min-w-full divide-y divide-gray-200 table-fixed dark:divide-gray-600">
                            ...
                            <tbody class="bg-white divide-y divide-gray-200 dark:bg-gray-800 dark:divide-gray-700">
                              ...
                              @foreach($products as $p)
                                ...
                                <tr class="hover:bg-gray-100 dark:hover:bg-gray-700">
                                    <td class="w-4 p-4">
                                      <div class="text-base font-semibold text-gray-900 dark:text-white">{{$number}}</div>
                                    </td>
                                    ...
                                    <td class="p-4 space-x-2 whitespace-nowrap ">
                                      <div class="flex  my-auto">
                                        <form style="margin-block-end: 0;" action="{{ route('product.restore', $p->id) }}" method="POST" >
                                            @csrf
                                            @method('PUT') <!-- Tambahkan method field -->
                                            <button type="submit" class="focus:outline-none text-white bg-green-400 hover:bg-green-800 focus:ring-4 focus:ring-green-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 dark:bg-green-600 dark:hover:bg-green-800 dark:focus:ring-green-900">Store</button>

                                        </form>
                                                                          
                                        <form style="margin-block-end: 0;" id="deleteForm_{{ $p->id }}" action="{{ url("/admin_panel/product/bin/{$p->id}") }}" method="POST" >
                                            @csrf
                                            @method('DELETE')
                                            <!-- Tombol untuk mengeksekusi penghapusan -->
                                            <a href="#" class="deleteButton text-white bg-red-600 hover:bg-red-800 focus:ring-4 focus:ring-red-300 font-medium rounded-lg text-sm inline-flex items-center px-3 py-2.5 text-center mr-2 dark:focus:ring-red-900" data-id="{{ $p->id }}" data-name="{{ $p->name_products }}">
                                                Permanent Delete
                                            </a>
                                        </form>
                                    </div>
                                      ...
                                      document.addEventListener('DOMContentLoaded', function() {
                                          ...
                                          deleteButtons.forEach(function(button) {
                                              button.addEventListener('click', function(event) {
                                                  event.preventDefault(); // Hindari default behavior link
                                                  var productId = this.getAttribute('data-id');
                                                  var productName = this.getAttribute('data-name');
                                                  productNameElement.textContent = productName;
                                                  deleteConfirmation.style.display = 'block'; // Tampilkan pop-up
                                                  confirmDeleteButton.onclick = function() {
                                                      var form = document.getElementById('deleteForm_' + productId);
                                                      form.submit(); // Submit formulir jika tombol Yes, Delete diklik
                                                  };
                                              });
                                          });
                                    
                                          cancelDeleteButton.addEventListener('click', function() {
                                              deleteConfirmation.style.display = 'none'; // Sembunyikan pop-up jika tombol Cancel diklik
                                          });
                                      });
                                    </script>
                                  </td>
  ...
  <div id="deleteConfirmation" tabindex="-1" class="hidden overflow fixed inset-0 z-50 bg-black bg-opacity-50">
    <div class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
      <div class="relative max-w-lg p-4 bg-white rounded-lg shadow-lg dark:bg-gray-800">
          <div class="p-4">
              <h3 class="mb-3 text-2xl font-bold text-center text-gray-900 dark:text-white">Delete Confirmation</h3>
              <p class="mb-4 text-sm font-light text-center text-gray-500 dark:text-gray-400">
                  Are you sure you want to permanently delete "<span id="productName"></span>" from the recycle bin?
              </p>
              <div class="flex justify-center space-x-4">
                  <button id="cancelDelete" type="button" class="py-2 px-4 w-full text-sm font-medium text-gray-500 bg-white rounded-lg border border-gray-200 hover:bg-gray-100 focus:ring-4 focus:outline-none focus:ring-primary-300 hover:text-gray-900 focus:z-10 dark:bg-gray-700 dark:text-gray-300 dark:border-gray-500 dark:hover:text-white dark:hover:bg-gray-600 dark:focus:ring-gray-600">Cancel</button>
                  <button id="confirmDelete" type="button" class="py-2 px-4 w-full text-sm font-medium text-white rounded-lg bg-red-600 hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-red-300 dark:bg-red-700 dark:hover:bg-red-800 dark:focus:ring-red-900">Yes, Delete</button>
              </div>
          </div>
      </div>
  </div>
