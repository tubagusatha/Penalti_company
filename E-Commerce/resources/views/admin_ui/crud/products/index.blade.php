    ...
    <div class="w-full mb-1">
        ...
        <div class="items-center justify-between block sm:flex md:divide-x md:divide-gray-100 dark:divide-gray-700">
            <div class="flex items-center mb-4 sm:mb-0">
              <form class="sm:pr-3" action="{{ url('admin_panel/product') }}" method="GET">
...
<!-- Pesan Sukses -->
@if (session('success'))
<div id="successMessage" class="px-4 py-2 bg-green-500 text-white">
    {{ session('success') }}
</div>
@endif
                    ...
                    @foreach($products as $p)
                      ...
                      <tr class="hover:bg-gray-100 dark:hover:bg-gray-700">
                          ...
                          <td class="p-4 text-sm font-normal text-gray-500 whitespace-nowrap dark:text-gray-400">
                            ...
                            <td class="max-w-sm p-4 overflow-hidden text-base font-normal text-gray-500 truncate xl:max-w-xs dark:text-gray-400">{{$p->detail_products}}</td>
                            ...
                            <td class="p-4 text-base font-medium text-gray-900 whitespace-nowrap dark:text-white">{{$p->category->name_category}}</td>
                        ...
                        @endif
                          ...
                          <td class="p-4 space-x-2 whitespace-nowrap">
                            ...
                            <a href="{{ url('admin_panel/product/' . $p->id . '/update') }}"
                              class="inline-flex items-center px-3 py-2 text-sm font-medium text-center text-white rounded-lg bg-primary-700 hover:bg-primary-800 focus:ring-4 focus:ring-primary-300 dark:bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-primary-800">
...
<script>
  ...
  var deleteForms = document.querySelectorAll('[id^="deleteProductForm_"]');

  // Mengaitkan event listener untuk setiap form penghapusan
  deleteForms.forEach(function(form) {
      form.addEventListener('submit', function(event) {
          // Menghentikan perilaku default form
          event.preventDefault();

          // Mendapatkan ID produk dari form action
          var productId = form.getAttribute('action').split('/').pop();

          // Membuat URL aksi penghapusan dengan ID produk yang benar
          var actionUrl = '/admin_panel/product/' + productId;

          // Mengirimkan permintaan DELETE menggunakan Fetch API
          fetch(actionUrl, {
              method: 'DELETE',
              headers: {
          ...
          .then(response => {
              ...
              window.location.href = '/admin_panel/product';
          })
          .catch(error => {
...
<div id="drawer-disabled-backdrop" class="fixed top-0 right-0 z-40 w-full h-screen max-w-xs p-4 overflow-y-auto transition-transform translate-x-full bg-white dark:bg-gray-800" tabindex="-1" aria-labelledby="drawer-disabled-backdrop-label" aria-hidden="true">
    ...
    <form action="{{ url('/admin_panel/product') }}" method="POST" enctype="multipart/form-data" class="h-full flex flex-col">
        @csrf
