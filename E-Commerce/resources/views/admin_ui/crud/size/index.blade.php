                        ...
                        <nav class="flex mb-5" aria-label="Breadcrumb">
                            <ol class="inline-flex items-center space-x-1 text-sm font-medium md:space-x-2">
                              ...
                              <li>
                                <div class="flex items-center">
                                  <svg class="w-6 h-6 text-gray-400" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z" clip-rule="evenodd"></path></svg>
                                  <span class="ml-1 text-gray-400 md:ml-2 dark:text-gray-500" aria-current="page">Size</span>
                                </div>
                              </li>
                            </ol>
                        </nav>
                        <h1 class="text-xl font-semibold text-gray-900 sm:text-2xl dark:text-white">All Sizes</h1>
                    </div>
                        ...
                        <table id="crudTable" class="min-w-full divide-y divide-gray-200 table-fixed dark:divide-gray-600">
                            ...
                            <tbody class="bg-white divide-y divide-gray-200 dark:bg-gray-800 dark:divide-gray-700">
                              ...
                              <tr class="hover:bg-gray-100 dark:hover:bg-gray-700">
                                  <td class="w-4 p-4 text-sm font-normal text-gray-500 whitespace-nowrap dark:text-gray-400">
                                  ...
                                  <td class="p-4 text-sm font-normal text-gray-500 whitespace-nowrap dark:text-gray-400">
                                    @if($s->product)
                                        @if(!empty($s->product->name_products))
                                            <div class="text-base font-semibold text-gray-900 dark:text-white">
                                                {{ $s->product->name_products }}
                                            </div>
                                        @else
                                            <h1>No Product</h1>
                                        @endif
                                  ...
                                  <td class="p-4 space-x-2 whitespace-nowrap">
                                      <form action="{{ url("/admin_panel/sizechart/$s->id") }}" method="POST">
                                          @csrf
                                          @method('DELETE')
                                          <!-- Button to execute the delete action -->
...
<div id="drawer-disabled-backdrop" class="fixed top-0 right-0 z-40 w-full h-screen max-w-xs p-4 overflow-y-auto transition-transform translate-x-full bg-white dark:bg-gray-800" tabindex="-1" aria-labelledby="drawer-disabled-backdrop-label" aria-hidden="true">
    <h5 id="drawer-disabled-backdrop-label" class="inline-flex items-center mb-6 text-sm font-semibold text-gray-500 uppercase dark:text-gray-400">New Size</h5>
   ...
   <form action="{{ url('/admin_panel/sizechart/create') }}" method="POST" enctype="multipart/form-data">
    @csrf
    <div class="space-y-4">
      <div>
        <label for="product_ids" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Select Products</label>
        <select id="product_ids" name="product_ids[]" multiple class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500">
            @foreach($products as $product)
                <option value="{{ $product->id }}">{{ $product->name_products }}</option>
            @endforeach
        </select>
    </div>
        <div>
            ...
            <label for="ukuran" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Add Size Product</label>
            <input type="text" name="ukuran" id="ukuran" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="Type size product" required="">
        </div>
